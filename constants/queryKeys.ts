export const AUTH = 'get@auth';

export const GET_USER = 'get@user';

export const GET_GENRE = 'get@blog/genre';

export const GET_BLOG = 'get@blog';

export const GET_AUTH_BLOGS = 'get@auth/blog';

export const GET_BOOKMARKS = 'get@auth/blog/bookmarks';

export const GET_LIKED_BLOGS = 'get@auth/blog/liked';

export const GET_USER_BLOGS = 'get@user/blog';

export const GET_FOLLOWERS = 'get@auth/followers';

export const GET_FOLLOWING = 'get@auth/following';

...
const Profile = () => {
  ...
  const { key, cachedKey, sort, sortOrder, genre, pageSize, isPublished, search } = useSelector(
  ...
  const { data: blogs, isLoading: isBlogsLoading } = useQuery({
    queryFn: () => authAxios.getAllBlogs({ ...getQueryArgs(getMenuKey(isPublished)), isPublished }),
    queryKey: [GET_AUTH_BLOGS, { ...getQueryArgs(getMenuKey(isPublished)), isPublished }],
  });

  const { data: bookmarkedBlogs, isLoading: isBookmarkedBlogsLoading } = useQuery({
    queryFn: () => authAxios.getBookmarkedBlogs(getQueryArgs(BOOKMARKED)),
    queryKey: [GET_BOOKMARKED_BLOGS, getQueryArgs(BOOKMARKED)],
    enabled: cachedKey.includes(BOOKMARKED),
  });

  const { data: likedBlogs, isLoading: isLikedBlogsLoading } = useQuery({
    queryFn: () => authAxios.getLikedBlogs(getQueryArgs(LIKED)),
    queryKey: [GET_LIKED_BLOGS, getQueryArgs(LIKED)],
          ...
          label: (
            ...
            >
              <Space direction='vertical'>
                ...
                <Radio value={CREATED_AT}>Latest</Radio>
                <Radio value={UPDATED_AT}>Last Updated</Radio>
              </Space>
            </Radio.Group>
          ),
        },
    ...
    <Menu
      ...
      items={
        ...
        genreSelector.map((val) => ({
          ...
          label: (
            <Checkbox
              defaultChecked={genre[key].includes(val)}
              value={val}
      ...
      children: (
        <>
          <span className='w-full flex gap-3 items-center sm:px-8 pt-2 sticky top-2 z-10'>
            ...
            <Dropdown overlay={menuGenre(key)}>
              <Button className='!bg-gray-200 border-gray-200 !text-black rounded-lg text-sm flex items-center gap-2 px-2'>
                <span>Genre:</span>
                <GrRadialSelected />
              </Button>
  ...
  return (
    <div className='w-full flex justify-center'>
      ...
      {authUser && (
        <main className='2xl:w-3/4 w-full flex flex-col items-center sm:gap-2 gap-2 py-2'>
          <div className='w-full flex sm:flex-row flex-col items-center justify-center sm:gap-14 gap-4'>
            ...
            <div className='sm:w-[55%] w-full flex flex-col sm:items-start items-center gap-3.5 break-all'>
              ...
              {authUser.bio ? (
                ...
                <p className='underline underline-offset-2 cursor-pointer inline-flex items-center gap-1 hover:text-gray-400 transition-all'>
